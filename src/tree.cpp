...
std::istream& operator>>(std::istream& is, tree& t)  //UPDATE: Added double T to inputs for streaming in means.
{
   ...
   for(size_t i=0;i!=nn;i++) {
      ...
      // Initialize helper variables for reading in m vector from istream.
      double m_temp;
      Rcpp::NumericVector m_vec_temp;

      while(is >> m_temp){
         m_vec_temp.push_back(m_temp);
      }

      // Convert NumericVector to arma and save to nv[i].m;
      nv[i].m = as<arma::vec>(wrap(m_vec_temp));
